...
class ImageService:
  ...
  def __init__(self, results, image_name):
    ...
    self.Image = ImageScene.ImageScene()
    self.ImageObjects = []
  ...
  def UpdateImage(self, identifier):
    ...
    self.Image = ImageScene.ImageScene(identifier, current_time)
    return image_id
  ...
  def UpdateBoundingBoxes(self, object_id, result):
    ...
    self.DataAccess.InsertBoundingBox(object_id, midpoint, length, width)
  ...
  def UpdateGeneralObject(self, result, image_id, image_identifier):
    ...
    return self.DataAccess.InsertObject(image_id, result['label'], result['confidence'], image_path)
